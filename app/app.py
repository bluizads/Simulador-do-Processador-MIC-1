...
class App:
    ...
    def __init__(self, source: str) -> None:
        self.source = self._get_source_text(source)
        pygame.init()
        ...
        # Fontes
        self.f_big   = App.font(17, bold=True)
        self.f_title = App.font(14, bold=True)
        self.f_label = App.font(12, bold=True)
        self.f_mono  = App.font(12)
        self.f_small = App.font(10)
        self.f_tiny  = App.font(9)
        ...
        self.editor_text   = self.source
        self.editor_cursor = len(self.source)
        self.editor_msg    = ""
        ...
        # Carga inicial
        self._assemble_and_load(close_editor=False)

        # Layout dos registradores no diagrama
        self._build_reg_layout()

    def _assemble_and_load(self, close_editor: bool = True) -> None:
        result = Assembler().assemble(self.source)
        if result.success:
    ...
    def _build_reg_layout(self) -> None:
        ...
        names = ["MAR", "MDR", "PC", "MBR", "", "SP", "LV", "CPP", "TOS", "OPC", "", "H"]
        ...
        for reg in names:
            if reg == "":
                ...
                self.reg_rects[reg] = pygame.Rect(30, pos_y, 108, 34)
                pos_y += 40
    ...
    def _draw_datapath(self) -> None:
        ...
        # Linhas dos registradores para os barramentos e memória
        for name, rect in self.reg_rects.items():
            if name in ["MAR", "MDR", "PC", "MBR"]:
                r = rect.move(ax + 30, ay)
                pygame.draw.line(self.screen, (255, 255, 255), (r.right, r.centery), (r.right + 100, r.centery), 1)
        for name, rect in self.reg_rects.items():
            if name in ["MAR", "H"]:
                continue
            r = rect.move(ax, ay)
            pygame.draw.line(self.screen, BUS_B, (r.right + 30, r.centery+4), (bus_bx, r.centery+4), 1)
        ...
        self._txt("A", bus_ax+4, 520, self.f_small, BUS_A)
        self._txt("B", bus_bx+4, top_y+150, self.f_small, BUS_B)
        ...
        # Barramento C
        bus_cx = alux + aluw + 22
        pygame.draw.line(self.screen, BUS_C, (alux+aluw//2, alu_y+aluh), (alux+aluw//2, alu_y+aluh + 10), 3)
        pygame.draw.line(self.screen, BUS_C, (alux+aluw//2, alu_y+aluh + 10), (bus_cx, alu_y+aluh + 10), 3)
        pygame.draw.line(self.screen, BUS_C, (bus_cx, alu_y+aluh + 10), (bus_cx, top_y), 3)
        self._txt("C", bus_cx+4, top_y+150, self.f_small, BUS_C)
        for name, rect in self.reg_rects.items():
            if name in ["MBR"]:
                continue
            r = rect.move(ax, ay)
            pygame.draw.line(self.screen, BUS_C, (r.right + 30, r.centery-5), (bus_cx, r.centery-5), 1)
        ...
        mw  = ax + aw - mx - 8
        if mw > 40:
            mem_r = pygame.Rect(mx, ay+28, mw, 160)
